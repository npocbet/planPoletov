...
Процедура ОтправитьНаПочтуДиректоруНаСервере()
	...
	Иначе 
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	ПолучателиОтчетовПоПочте.ПолучательОбщийОтчетЗаСутки = ИСТИНА
			|	И ПолучателиОтчетовПоПочте.ПометкаУдаления = ЛОЖЬ";  			
		РезультатЗапроса = Запрос.Выполнить();
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Ответ = Вспомогательные.ОтправитьСообщение(
		    ВыборкаДетальныеЗаписи.АдресПолучателя, "Общий отчет за "+ПараметрСКДДата, "", ИмяТемпФайла,
			...
			Если Ответ Тогда 
				...
				МенеджерЗаписи.ТипОтправляемогоДокумента = Перечисления.ТипОтправляемогоДокумента.ОбщийОтчетЗаСутки;
...
Процедура ПриОткрытииНаСервере()
	...
	Запрос.УстановитьПараметр("ТипОтправляемогоДокумента", Перечисления.ТипОтправляемогоДокумента.ОбщийОтчетЗаСутки);
